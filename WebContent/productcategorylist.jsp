		...
		<c:if test="${pageBean != null && pageBean.totalPage != 0}">
			<div class="pageBean">
				...
				<c:if test="${pageBean.currentPage != 1}">
					<a style="width: 80px;" href="<%=basePath%>/productList?keyword=${keyword}&currentPage=${pageBean.currentPage - 1}&cid=${cid}">
						<i><</i>
						<span>上一页</span>
					</a>
				</c:if>
				...
				<c:forEach begin="${pageBean.start}" end="${pageBean.end}" step="1"
					...
					<c:if test="${pageBean.currentPage != i}">
						<a href="<%=basePath%>/productList?keyword=${keyword}&currentPage=${i}&cid=${cid}">${i}</a>
					</c:if>
				</c:forEach>
				<!-- 3.显示下一页 -->
